...
class JiraRestClient
{
    ...
    const JIRA_URL = 'https://jira.url.com/';
    ...
    public static function getInstance()
    {
        if (empty(self::$instance)) {
            ...
            self::$instance->setJiraUrl(self::JIRA_URL);
        }
    ...
    public function setFieldValue($issue_key, $field_name, $value)
    {
        $field_info = $this->getFieldInfo($field_name);

        if ($field_info['custom']) {
            $field_type = explode(':', $field_info['schema']['custom']);
            $field_type = array_pop($field_type);
            if (in_array($field_type, ['float', 'string', 'textarea', 'textfield']) || is_array($value)) {
                $update = ['fields' => [$field_info['id'] => $value]];
            } else {
                $update = ['fields' => [$field_info['id'] => ['value' => $value]]];
            }
        } else {
            $update = ['fields' => [$field_name => $value]];
        }
